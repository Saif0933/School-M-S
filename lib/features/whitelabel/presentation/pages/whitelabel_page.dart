...
class _WhiteLabelPageState extends ConsumerState<WhiteLabelPage>
  ...
  final _longUrlCtrl = TextEditingController(
    text:
        'https://sunrise.symbosys.edu/delhi/canteen/ordering/ticket/129380128',
  );
  ...
  void _loadBranchConfig() {
    ...
    final c = configs.firstWhere(
      (element) => element.branchId == _selectedEditBranchId,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final emailTemplates = ref
        .watch(emailTemplatesProvider)
        .where((t) => t.branchId == _selectedEditBranchId)
        .toList();
            ...
            child: TabBar(
              ...
              tabs: const [
                Tab(
                  ...
                  text: 'Branch Domain & Colors Override',
                ),
                Tab(
                  icon: Icon(Icons.mail_outline_rounded, size: 16),
                  text: 'Email & SMS templates customizer',
                ),
                Tab(
                  icon: Icon(Icons.visibility_off_rounded, size: 16),
                  text: 'Menu visibility & URL shortener',
                ),
              ],
            ),
          ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final formWidget = Column(
          ...
          children: [
            const Text(
              '✏️ Branch Branding configurations',
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Branch Sub-Domain (e.g. delhi.org.com)',
              ),
            ),
            TextField(
              controller: _logoCtrl,
              decoration: const InputDecoration(
                labelText: 'Branch Logo file override name',
              ),
            ),
            TextField(
              controller: _smsIdCtrl,
              decoration: const InputDecoration(
                labelText: 'Branch SMS Sender ID (6 Characters)',
              ),
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              ...
              decoration: const InputDecoration(
                labelText: 'Custom Terminology Mapping',
              ),
              items: const [
                DropdownMenuItem(
                  value: 'Class/Teacher',
                  child: Text('Standard (Class, Section, Teacher)'),
                ),
                DropdownMenuItem(
                  value: 'Grade/Educator',
                  child: Text('Modern International (Grade, Group, Educator)'),
                ),
              ],
              onChanged: (val) =>
                  setState(() => _selectedTerm = val ?? 'Class/Teacher'),
            ),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              initialValue: _selectedColor,
              decoration: const InputDecoration(
                labelText: 'Custom Branch Color Theme',
              ),
              items: const [
                DropdownMenuItem(
                  value: '#4F46E5',
                  child: Text('Classic Indigo Blue (#4F46E5)'),
                ),
                DropdownMenuItem(
                  value: '#0D9488',
                  child: Text('Emerald Green (#0D9488)'),
                ),
                DropdownMenuItem(
                  value: '#D97706',
                  child: Text('Sunset Orange (#D97706)'),
                ),
              ],
              onChanged: (val) =>
                  setState(() => _selectedColor = val ?? '#4F46E5'),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  ref
                      .read(branchWhiteLabelProvider.notifier)
                      .updateConfig(_selectedEditBranchId, updated);
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        '✓ Custom branch branding overrides saved.',
                      ),
                    ),
                  );
                },
        ...
        final previewWidget = Column(
          ...
          children: [
            const Text(
              '🛡️ Branch login page white-label preview',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
            ),
            const SizedBox(height: 12),
            Container(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 16),
                  Text(
                    'Sub-Domain: ${_subDomainCtrl.text}',
                    style: const TextStyle(fontSize: 10, color: Colors.grey),
                  ),
                  Text(
                    'SMS Sender ID: ${_smsIdCtrl.text}',
                    style: const TextStyle(fontSize: 10, color: Colors.grey),
                  ),
                  const SizedBox(height: 12),
                  const TextField(
  ...
  Widget _buildEmailTab(List<BrandedEmailTemplate> templates) {
    if (templates.isNotEmpty) {
      final t = templates.firstWhere(
        (element) => element.id == _selectedEmailTemplateId,
      ...
      child: Center(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              const Text(
                '✏️ Custom Email Notification templates builder',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
              ),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                ...
                decoration: const InputDecoration(
                  labelText: 'Notification Trigger Trigger',
                ),
                items: const [
                  DropdownMenuItem(
                    value: 'FEE-ALERT',
                    child: Text('Fees collection alert email template'),
                  ),
                ],
                onChanged: (val) {
                  if (val != null) {
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Custom Subject Header',
                ),
              ),
              TextField(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('✓ Custom email trigger template saved.'),
                      ),
                    );
                  },
                  child: const Text(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final togglesWidget = Column(
          ...
          children: [
            const Text(
              '👁️ Branch Menu Visibility controls',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
            ),
            const SizedBox(height: 12),
            ...modulesList.map((modName) {
              ...
              return CheckboxListTile(
                ...
                subtitle: const Text(
                  'Show in branch navigation sidebars',
                  style: TextStyle(fontSize: 9),
                ),
                value: !isHidden,
                onChanged: (val) {
                  final updatedList = List<String>.from(
                    activeConfig.hiddenMenuIds,
                  );
                  if (val == true) {
                    updatedList.remove(modName);
                  } else {
                    if (!updatedList.contains(modName))
                      updatedList.add(modName);
                  }
                  ref
                      .read(branchWhiteLabelProvider.notifier)
                      .updateConfig(
        ...
        final shortenerWidget = Column(
          ...
          children: [
            const Text(
              '🔗 White-Label custom URL Shortener',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13),
            ),
            const SizedBox(height: 8),
            const Text(
              'Generate short custom links using your custom domains.',
              style: TextStyle(color: Colors.grey, fontSize: 10),
            ),
            const SizedBox(height: 12),
            TextField(
              controller: _longUrlCtrl,
              decoration: const InputDecoration(
                labelText: 'Long URL Address Link',
              ),
            ),
            const SizedBox(height: 12),
            SizedBox(
